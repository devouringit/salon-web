...
function FeedbackPage({ storeData, metaTags }) {
    ...
    useEffect(() => {
        if (storeData.appointmentId) {
            getAppointmentById(storeData.appointmentId).then((appointmentDetails: any) => {
                if (appointmentDetails.guestName)
                    setappointmentData(appointmentDetails);
                getFeedbackByAppointmentId(storeData.appointmentId).then((feedbackDetails: any) => {
                    if (feedbackDetails) {
    ...
    }, [storeData.appointmentId])

    useEffect(() => {
        if (storeData.appointmentId && appointmentData && appointmentData.id) {
            const doneServicesList = appointmentData.expertAppointments;
            if (doneServicesList) {
                doneServicesList.map((service) => {
                    service.feedbackConfig = configData.feedbackConfig;
                })
        ...
        <div className="background-wrapper">
            ...
            <div className="feedback-container">
                <div className="feedback-page-wrap">
                    {configData?.feedbackOn ?
                        <>
                            {feedbackAlreadySubmitted ? <>
                                <div className="feedback-c-main-wrap">
                                    <div className="feedback-c-wrap">
                                        <div><img className="submit-logo" src={`/assets/images/${gender}/order_confirm.png`} style={{ width: '70%' }} /></div>
                                                                    ...
                                                                    {serviceData.feedbackConfig.typeList.map((feedbackType: any, feedbackTypeIndex: number) => {
                                                                        return <div key={feedbackTypeIndex}>
                                                                            ...
                                                                            {feedbackType.entity == "store" && feedbackType.active && feedbackType.canComment &&
                                                                                <div> <div className="remarkTitle">Remark <span className="font-ifAny">(if any)</span></div>
